...
#if OS_FUCHSIA
#if OS_FUCHSIA
  public void UpdateScene(SceneUpdateContext context, scenic.ContainerNode container)
  {
	TRACE_EVENT0("flutter", "LayerTree::UpdateScene");
	auto metrics = context.metrics();
	SceneUpdateContext.Transform transform = new SceneUpdateContext.Transform(context, 1.0f / metrics.scale_x, 1.0f / metrics.scale_y, 1.0f / metrics.scale_z);
	SceneUpdateContext.Frame frame = new SceneUpdateContext.Frame(context, SkRRect.MakeRect(SkRect.MakeWH(frame_size_.width(), frame_size_.height())), new uint32_t(GlobalMembers.SK_ColorTRANSPARENT), 0.0f);
